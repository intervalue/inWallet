...
angular.module('copayApp.services').factory('backButton', function($log, $rootScope, gettextCatalog, $deepStateRedirect, $document, $timeout, go, $state, lodash) {
	...
	$rootScope.$on('$stateChangeSuccess', function(event, to, toParams, from, fromParams){
		...
		if (to.name == "walletHome" || to.name == "discovery" || to.name == "exchange" || to.name == "correspondentDevices" || to.name == "wallet") {
	...
	function back() {
        let tabFrom = $rootScope.tabFrom;
        if(tabFrom){
            $rootScope.tabFrom =''
            $rootScope.$emit('Local/createTabChange',tabFrom)
			return
        }
        if (body.hasClass('modal-open')) {
            ...
            if ((!currentState || currentState.from == "") && !tabFrom ) {
